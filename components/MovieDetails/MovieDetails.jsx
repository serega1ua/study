...
import {setIsSelected} from "../../app/features/moviesReducer";

const MovieDetails = () => {
  const movie = useSelector((state) => state?.selectedMovie);
  const dispatch = useDispatch();
  return (
    <div id="details" className="flex flex-col items-center bg-black justify-around py-8">
      <header className="flex justify-between w-details py-3">
        <span className="text-xl text-lightred">netflixroulette</span>
        <button onClick={() => dispatch(setIsSelected(false))}>
          <svg
      ...
      <div className="w-details flex border-lightgray border-b-8 pb-5">
        ...
        <div className="flex flex-col ml-5">
          ...
          <p className="text-white opacity-50 mb-4">{movie.genres?.join(" ")}</p>
          <div className="flex justify-start mb-5">
            <span className="text-lightred text-xl mr-4">{movie.release_date}</span>
            <span className="text-lightred text-xl">{movie.runtime}</span>
          </div>
          <div>
            <p id="overview" className="text-white opacity-50 text-xl">{movie.overview}</p>
          </div>
        </div>
      </div>
    </div>
