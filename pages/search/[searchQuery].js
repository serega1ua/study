...
import DeletePopup from "../../components/SearchedMoviesLayout/Main/components/Popup/DeletePopup";
import Popup from "../../components/SearchedMoviesLayout/Main/components/Popup/Popup";
import SuccessAddedPopup from "../../components/SearchedMoviesLayout/Main/components/Popup/SuccessAddedPopup";
import MainSection from "../../components/SearchedMoviesLayout/Main/MainSection";
import { Context } from "../../context/MainContext";
import { useDispatch, useSelector } from "react-redux";
import {fetchMovies, getMovieDetails, searchMovies, setMovies} from "../../app/features/moviesReducer";
...
const SearchedMovies = ({data}) => {
    ...
    const selectMovie = (movie) => {
        dispatch(getMovieDetails({id: movie.id}));
        navigate.push(`/search/${searchQuery}?movie=${movie.id}`)
    };
                    ...
                    <div className="flex flex-col justify-between items-start">
                        ...
                        <div className="flex flex-wrap justify-between">
                            {reduxMovies.map((movie) => (
                                <div id="searched-movie" key={movie.id} className="mt-6">
                                    <div className="group relative">
                                        <img
                                            src={movie.poster_path}
                                            alt={movie.title}
                                            id={`searched-movie-${movie.title}`}
...
export const getServerSideProps = async(context) => {
    ...
    const res = await fetch(`http://localhost:4000/movies?search=${searchQuery}&searchBy=title&filter=&sortBy=&sortOrder=`, {
        headers: {
...
/*export const getStaticPaths = async ({params}) => {
    ...
    const res = await fetch(`http://localhost:4000/movies?search=${params?.searchQuery}&searchBy=title&filter=&sortBy=&sortOrder=`, {
        headers: {
            'Access-Control-Allow-Credentials' : true,
            'Access-Control-Allow-Origin':'*',
            'Access-Control-Allow-Methods':'GET',
    ...
    const paths = data.data.map((movie) => ({
        params: {searchQuery: movie.title}
    }))
    return {
        paths,
        fallback: false
